...
```{r}
total2 <-merge(total,Data, by="subject")%>%
  select(-mean_unique,-`mean_overlapping`)
  head(total2)
anova2 <-ezANOVA(data =total2,
                   dv = accuracy,
                   wid = subject,
                   within = .(overlapping,unique),
                   detailed = T,
                   return_aov = T)%>%
kable(savings_anova$ANOVA[1:7], digits = 3, padding=10, caption = "Savings ANOVA") %>%
